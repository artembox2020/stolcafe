    ...
    <script>
        ...
        function get_add_to_cart(id,style) {
          ...
          form.on('submit',function(e){
            ...
            $.get("/?"+$(this).serialize()+"&add-to-cart="+id, function() { 
...
var btn = form.find('button'); btn.removeAttr('disabled'); btn.find("a").css('pointerEvents','auto'); btn.find("a").html('Просмотр корзины'); });
            $(this).find("button").attr('disabled','disabled');
            $(this).find("button").html("<a style='pointer-events:none;' href='<?php echo wc_get_cart_url(); ?>'>Обработка...</a>");
          });
           ...
           function animateMargins(selector) {
               ...
               animateH = setInterval(function(){
                   if(parseInt(selector.css('marginLeft')) < -48) { 
                       clearInterval(animateH); 
                       selector.find(".dfd-price-block-id").fadeOut("400");
                       setTimeout(function() { selector.css('marginLeft','0'); } , 300);
                       return true; 
                   }
                   selector.css('marginLeft', ( parseInt( selector.css('marginLeft') ) - 5 )+"px" );
               },40);
           }
           
           
           
           function make_body2(cat_id,page_id,is_navbar){
           ...
           //$(sel).html('');
           //$(window).resize();
           if(!is_navbar) $("div.page-nav.text-center").remove();
           //$('nav.page-nav.text-center').remove();
                   ...
                   for(var j = 1; j <= number_pages; ++j ) {
                       if(j == current_page) {
                         ...
                         arf[j].on('click',function(e){
                             ...
                             make_body2($(this).attr('catid'),$(this).attr('pgid'),true);
                         });
                         lis+= '<li arfid="'+j+'"></li>';
                       }
                   }
	...
	'</nav><div class="dfd-next-page"><span prvblock="nxt">'+'</span><i class="dfd-socicon-arrow-right-01"></i></a></div></div>').appendTo(nav);
	           ...
	           else nav.insertBefore(".vc_tta-panel-body .columns.six:eq(0)");
          ...
          $(".vc_tta-panel-body .columns.six").css('backgroundColor','white');
           ...
           <?php  for($i=0;$i<count($prods['cats']) - 1;++$i): ?>
