...
btn.find("a").html('Просмотр корзины'); 
           ...
           function make_days_tab()
           {
                ...
                $.ajax({
                    url: "/apis/get_days_week.php",
                    method: "GET",
                    async: false,
                    success: function(r) {
                        var res = JSON.parse(r);
                        var days = res.days.split(",");
                        var sign = false;
                        
                        for (var i = 0; i < days.length; ++i) {
                            var active = res.daysInfo[parseInt(days[i])];
                            switch (parseInt(days[i])) {
                                case 1: var slug = 'Mn'; var rSlug = 'ПН'; break;
                                case 2: var slug = 'Tu'; var rSlug = 'ВТ'; break;
                                case 3: var slug = 'Wn'; var rSlug = 'СР'; break;
                                case 4: var slug = 'Th'; var rSlug = 'ЧТ'; break;
                                case 5: var slug = 'Fr'; var rSlug = 'ПТ'; break;
                            }
                            var li = make_tab_days(rSlug, slug, cat_id, 0, 0-parseInt(days[i]), active);
                            li.appendTo(tabsList);
                        }

                        var lis = tabsList.find('li'); 
                        for (var i = 0; i < lis.length; ++i) {
                            if (!lis[i].classList.contains('disabled')) {
                                setTimeout(function() { 
                                    //lis[i].click();
                                    setTimeout(function() { lis[i].click(); }, 500);
                                }, 4000);
                                break;
                            }
                        }
...
var btn = form.find('button'); btn.removeAttr('disabled'); btn.find("a").css('pointerEvents','auto'); btn.find("a").html('Просмотр корзины'); },400);
  ...
  var span = $('<span class="vc_tta-title-text">');
           ...
           */
           ...
           function make_tab_days(name,id,catid,active, day, accessible) {
             ...
             li.click(function(e) {
                   ...
                   //$(this).closest('ul').attr('style', "display:block!important;");
           ...
           {
                var sel2 = ".vc_tta-tabs-list:eq(0)";
                sel2 = ".vc_tta-panels-container:eq(0)";
                var tabsList = $("<ul class='vc_tta-tabs-list-days'>");
                var a = $(sel2).find("li.vc_active a");
                ...
                $.ajax({
                    ...
                    success: function(r) {
                        ...
                        var days = res.days.split(",");
                        for (var i = 0; i < days.length; ++i) {
                            var accessible = res.daysInfo[parseInt(days[i])];
                            switch (parseInt(days[i])) {
                                case 1: var slug = 'Mn'; var rSlug = 'ПН'; break;
                                case 2: var slug = 'Tu'; var rSlug = 'ВТ'; break;
                                case 3: var slug = 'Wn'; var rSlug = 'СР'; break;
                            ...
                            var li = make_tab_days(rSlug, slug, cat_id, 0, 0-parseInt(days[i]), accessible);
                            li.appendTo(tabsList);
                        }
                        var lis = tabsList.find('li'); 
                        for (var i = 0; i < lis.length; ++i) {
        ...
        function make_body2(cat_id,page_id,selector, day_id) {
           ...
           $.ajax({ url:"/apis/get_prods.php", data:pars, method: "GET", async:true, success: function(r){
               var res = JSON.parse(r);
                   ...
                   for(var j = 1; j <= number_pages; ++j ) {
                       if(j == current_page)
                         ...
                         arf[j].on('click',function(e){
                             ...
                             var selecto2 = make_body2($(this).attr('catid'),$(this).attr('pgid'),"<div>", dayid);
               ...
               //if(current_page < number_pages) var nxt = '<a href="/?pg='+(current_page + 1)+'&cat_id='+cat_id+'">След.</a>'; else var nxt = 'След.';
                if(current_page > 1) { 
                   ...
                   prv.on('click',function(e){
                        ...
                        var selecto = make_body2($(this).attr('catid'),$(this).attr('pgid'),make_body_back(2,0,23),dayid);
	...
	'</nav><div class="dfd-next-page"><span prvblock="nxt">'+'</span><i class="dfd-socicon-arrow-right-01"></i></a></div></div>').appendTo(nav);
          ...
          function updateAllBody()
          {
            dvb.html('');  
            <?php

                for($i=0;$i<count($prods['cats']);++$i): 
            ?>
               ...
               var selecto = make_body2(<?= $prods['cats'][$i]['id'] ?>,<?php if(isset($_GET['pg'])) echo (intval($_GET['pg']) -1); else echo '0';  ?>,make_body_back(2,0,23), dayid);
